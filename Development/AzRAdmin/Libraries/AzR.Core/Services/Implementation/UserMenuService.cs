    ...
    public class UserMenuService : IUserMenuService
    {
        ...
        public IEnumerable<UserPrivilegeViewModel> GetByUserId(int id)
        {
            try
            {
                ...
                var usrInfo = from u in _user.GetAll
                              join r in _user.GetAll.Include(s => s.Roles).SelectMany(s => s.Roles) on u.Id equals r.UserId
                              join m in _menu.GetAll on r.RoleId equals m.RoleId
                              where u.Id == id
                ...
                var model = (from u in usrInfo
                             join mc in _menu.GetAll on u.MenuId equals mc.Id
                             join pp in _userPrivilege.GetAll on new { u.UserId, u.MenuId } equals new { pp.UserId, pp.MenuId } into lgj
        ...
        public UserMenu Create(UserPrivilegeViewModel model)
        {
            var itm = new UserMenu
            {
                UserId = model.UserId,
                MenuId = model.MenuId,
