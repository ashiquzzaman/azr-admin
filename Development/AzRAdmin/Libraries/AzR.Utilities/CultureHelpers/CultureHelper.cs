...
namespace AzR.Utilities.CultureHelpers
{
    public static class CultureHelper
    {
        ...
        public static string GetNeutralCultureLocation(string name)
        {
            if (!name.Contains("-")) return name;

            return name.Split('-')[1]; // Read first part only. E.g. "en", "es"
        }


        public static string GetCultureEnglishName(string name)
        {
            var ci = CultureInfo.GetCultureInfo(name);
            return ci.IsNeutralCulture ? ci.EnglishName : ci.Parent.EnglishName;
        }
        public static string GetCultureNativeName(string name)
        {
            var ci = CultureInfo.GetCultureInfo(name);
            return ci.IsNeutralCulture ? ci.NativeName : ci.Parent.NativeName;
        }
        ...
        public static string SetCulture(string culture)
        {
            ...
            HttpCookie cookie = HttpContext.Current.Request.Cookies["APPCULTURE"];
            if (cookie != null)
                cookie.Value = culture;   // update cookie value
            else
            {

                cookie = new HttpCookie("APPCULTURE")
                {
                    Value = culture,
